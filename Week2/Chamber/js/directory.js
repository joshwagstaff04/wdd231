...
function renderCard(m){
  ...
  card.innerHTML = `
    <div class="media">
      <img src="${escapeAttr(m.image)}" alt="${escapeAttr(m.name)} logo" loading="lazy" width="640" height="360" />
    </div>
    <div class="body">
      <h3 class="name">${escapeHTML(m.name)}</h3>
      <span class="address">${escapeHTML(m.address)}</span>
      <span class="phone"><a href="tel:${digits(m.phone)}">${escapeHTML(m.phone)}</a></span>
...
function badgeFor(level=1){
  const map = { 3: ['Gold','gold'], 2: ['Silver','silver'], 1: ['Member','member'] };
